...
void aeParser::parseValue(aeon_lexer& lexer, AEValue& rootValue)
{
	...
	{
		AEValue ParsedValue = parseDataValue();
...
AEValue aeParser::parseDataValue()
{
	...
	if (Tok.type == AETK_COLON)
	{
		...
		{
			// This value is a function body

		}
		else
		{
			// This value is array/number/expression
			val = ToValue(parsePropertyValue());
		}
	...
	else if (Tok.type == AETK_OPENBRACKET)
	{
		...
		while (Tok.text != "}")
		{
			AEValue ChildValue = parseDataValue();
