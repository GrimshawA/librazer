...
RzValue::RzValue(void* obj, RzType* typeInfo)
{
...
void RzValue::setValue(const std::string& name, const std::string& value)
{
	...
	else if (m_valueType == VALUE_PROTOTYPE)
	{
		_dynObj->setValue(name, value);
	}
	else if (!m_valueType)
	{
		m_valueType = VALUE_PROTOTYPE;
...
void RzValue::setFromObject(void* ptr, RzType* typeInfo)
{
	_object = new AEObject();
	_object->m_obj = ptr;
	_object->m_type = typeInfo;
