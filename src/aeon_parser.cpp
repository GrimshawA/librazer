...
aeon_parser::aeon_parser()
{
	...
	aeNodeBase* aeon_parser::parse_symbol()
	{
		...
		aeNodeTypeDecl* type_node = parse_type();
	...
	aeNodeTypeDecl* aeon_parser::parse_type()
	{
		aeNodeTypeDecl* type_node = new aeNodeTypeDecl();
		...
		if (Tok.type == Tok.MarkOpen)
		{
			...
			{
				aeNodeTypeDecl* template_type_node = parse_type();
...
aeNodeVarDecl* aeon_parser::parseVariableDecl()
{
	...
	aeNodeExpr* aeon_parser::parseExpression()
	{
		...
		do
		{
			...
			{
				aeNodeExpr* subexpr = parse_identifier_subexpression();
				leafexpr = subexpr;
			}
	...
	aeNodeExpr* aeon_parser::parse_identifier_subexpression()
	{
		...
		if (Tok.type == Tok.Period)
		{
			// There is more!
			getNextToken();
			aeNodeExpr* structmember = parse_identifier_subexpression();
