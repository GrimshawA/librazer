...
aeon_parser::aeon_parser()
{
	...
	void aeon_parser::parse(aeon_lexer& lexer)
	{
		...
		while (Tok.type != aeon_token::EndOfFile)
		{
			...
			{
				aeon_ast_function* funcDecl = parse_function();
				root->add(funcDecl);
			...
			{
				aeon_ast_node* symbol = parse_symbol();
				if (symbol)
				{
					root->add(symbol);
				}
	...
	void aeon_parser::parse_class_element(ast_class* classDeclNode)
	{
		...
		{
			// Constructor
			aeon_ast_function* constructorDecl = new aeon_ast_function();
			constructorDecl->m_name = classDeclNode->Name;
			getNextToken();
			getNextToken();
		...
		else if (Tok.type == Tok.Tilde)
		{
			...
			{
				aeon_ast_function* destructorDecl = new aeon_ast_function();
				destructorDecl->m_name = std::string("~") + classDeclNode->Name;
				getNextToken();
				getNextToken();
				getNextToken();
	...
	aeon_ast_node* aeon_parser::parse_symbol()
	{
		aeon_ast_node* result_node = nullptr;
		bool IsFunctionDecl = false;
		bool IsConst = false;
		std::vector<aeon_expression*> params;
		...
		if (Tok.type == Tok.BracketOpen)
		{
			if (IsFunctionDecl)
			{
				aeon_ast_function* func_node = new aeon_ast_function();
	...
	std::vector<aeon_expression*> aeon_parser::parse_argument_list()
	{
		std::vector<aeon_expression*> temp;

		aeon_expression* argExpr = nullptr;
...
std::vector<aeon_expression*> aeon_parser::parse_parameter_list()
{
	...
	{
		aeon_ast_function* funcDecl = new aeon_ast_function();
	...
	void aeon_parser::parseFunctionScope(aeon_ast_function* funcDeclNode)
	{
...
void aeon_parser::parseBlock(ast_codeblock* block)
{
		...
		while (Tok.type != Tok.BracketClose)
		{
			...
			{
				aeon_expression* expr = parseExpression();
				if (expr)
				{
					block->add(expr);
				}
...
ast_for* aeon_parser::parseForLoop()
{
	...
	aeon_expression* initStatement = parseExpression();
	if (initStatement)
...
ast_while* aeon_parser::parseWhileLoop()
{
...
aeon_stmt_vardecl* aeon_parser::parseVariableDecl()
{
	...
	aeon_expression* aeon_parser::parseExpression()
	{
		...
		std::vector<aeon_expression*> operands;
		...
		do
		{
			...
			aeon_expression* leafexpr = nullptr;
			...
			{
				aeon_expression* subexpr = parse_identifier_subexpression();
				leafexpr = subexpr;
			}
		...
		aeon_expression* lhs = operands[0];
		for (int i = 0; i < operators.size(); ++i)
		{
			aeon_expression* rhs = operands[i + 1];
			...
			{
				aeon_expression* rhs = new ast_binaryop(operands[i + 1], operands[i + 2], operators[i + 1].text);
				lhs = new ast_binaryop(lhs, rhs, operators[i].text);
				i++;
			}
		}
	...
	aeon_expression* aeon_parser::parse_identifier_subexpression()
	{
		aeon_expression* result_expr = nullptr;
		...
		if (Tok.type == Tok.SqBracketOpen)
		{
			...
			aeon_expression* subscript_expression = parseExpression();
		...
		if (Tok.type == Tok.Period)
		{
			// There is more!
			getNextToken();
			aeon_expression* structmember = parse_identifier_subexpression();
